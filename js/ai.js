...
export async function generateKnowledgeAnswer(question, taxonomy, options = {}) {
  ...
  const system = [
    ...
    'Adapt depth to the question: roughly 1800-2800 Japanese characters for a focused simple question, 2800-4200 for a standard question, and up to 6000 only for a genuinely complex question. Add explanatory depth, not decorative wording, repeated conclusions, or inflated phrasing.',
    ...
    'Finish the central explanation before extending it. Then develop one or two genuinely useful consequences, related mechanisms, changed conditions, applications, unresolved questions, or links to another field. Explain exactly how each extension follows from the central mechanism; do not append a generic related-topics list.',
    'Use the available Google Search tool to verify the central factual claims before finalizing. For scientific and academic claims prefer peer-reviewed papers, review articles, academic institutions, scholarly reference works, or official organizations. For current statistics, laws, standards, and public policy prefer the responsible primary official source.',
    'When reliable sources disagree, identify what is established and what remains uncertain. When evidence is limited, say so plainly and reduce the strength of the claim instead of filling the gap with a plausible story.',
    'Never invent a publication, author, quotation, statistic, URL, or citation. Do not place a bibliography or source URLs in the JSON: the application attaches only the grounding sources returned by the search tool.',
    ...
    'For a standard answer, use three to five content-specific sections with one to three natural paragraphs in each. A very focused question may use two sections. Keep each paragraph focused on one connected idea; avoid both a single wall of text and a heading for every short paragraph.',
    'Every section must keep its core explanation in paragraphs. It may also include richBlocks, but only when a visual structure materially improves understanding. Return richBlocks as an empty array otherwise.',
    'Use a list for genuinely parallel points or a real sequence, not to fragment ordinary prose. Use a table only for a comparison with shared axes and never invent values to fill cells.',
    'Use an equation only when the mathematical relationship matters. Supply valid KaTeX-compatible LaTeX without delimiters, an accurate plainText reading, and a short explanation of what the variables and relationship mean.',
    'Use a callout only for a compact definition, a consequential caveat, or one essential distinction. Use flow only for a causal chain, chronological sequence, or process with two to seven clear nodes. Every flow needs a complete altText that conveys the same meaning without the diagram.',
    'Do not use richBlocks as decoration and do not repeat the same content in a paragraph and a richBlock. Across the whole answer, prefer zero to three richBlocks.',
    ...
    'Use marks sparingly and semantically: strong for a defining term or the shortest essential claim, highlight-yellow for a core mechanism or causal relation, highlight-blue for a contrast, condition, or framing distinction, and warning only for a real caveat.',
    'When the answer supports it, place one short strong segment in the direct answer and two to four short marked phrases across the body. Keep each marked span brief and never mark an entire sentence or paragraph merely for decoration.',
...
export async function generateNuanceEntries(
  ...
  const system = [
    ...
    'For a Japanese learningTarget, include an English expression only when it naturally conveys that intent in a common identifiable construction. Distinguish a general equivalent from an expression that approaches the Japanese only through a special pragmatic use. Do not promote a context-dependent association such as even for そもそも into a default equivalent, and do not force weakly related entries merely to reach the target count.',
    ...
    'Count rules depend strictly on generationMode. In normal_set mode, return five complete expressions normally, at least four complete expressions unless accuracy makes that impossible, and never reduce the count merely because a loosely similar expression exists in existingCatalog. In saved_headword_enrichment mode only, the directly requested saved headword is mandatory and is the only count requirement: return one excellent complete headword entry (or several entries only for genuinely distinct parts of speech or semantic branches). Related comparison expressions are optional in saved_headword_enrichment mode and must be omitted when already saved or when adding them would reduce depth. Never apply this one-entry exception to a new headword or a merely similar headword.',
    ...
    'Return at least three natural example sentences for every expression, each with an idiomatic Japanese translation and a short usage note. Translate what the English sentence naturally means in that context; do not force learningTarget, the topic label, or the same Japanese gloss into every translation. A translation may omit, rephrase, or vary the searched Japanese wording when natural Japanese requires it. Put the explanation of how the English expression connects to the learner\'s request in noteJa or nuanceJa, not by distorting the translation. The examples must be genuinely different in situation, grammar, collocation, and communicative purpose; never reuse or lightly rephrase an example anywhere in the set.',
    'This natural-translation rule applies to every English example anywhere in the response, including main examples, usagePatterns, and collocations. First translate the complete English sentence independently into the Japanese a human translator would normally write. Use the exact learningTarget wording only when that particular sentence itself naturally calls for it; semantic similarity alone is not a reason to insert it.',
    ...
    'For collocations, return the English combination together with a short natural Japanese meaning for that exact combination. Add a concise usageNoteJa stating when the combination is natural or what it foregrounds, plus one or two distinct natural examples with idiomatic Japanese translations and a short sentence-specific note. Translate each sentence on its own terms rather than repeating the topic gloss. Translate the combination in context, not as isolated dictionary words. Do not inflate the list with combinations whose usage adds no learning value.',
    ...
    'For a normal set, give every expression at least one decisive direct comparison with another expression in the returned set, and add more comparisons when they reveal a genuinely different boundary. For saved-headword enrichment, provide at least two concrete comparisons. Explain differences in viewpoint, implication, grammar, strength, or register instead of giving interchangeable dictionary glosses.',
  ...
  const user = JSON.stringify({
    ...
    requestedEntryCount: hasRequestedSavedHeadword
