// タブレット・PC用CSSがスマートフォン表示やPWA設定へ漏れないことを守るテスト。
...
test('responsive stylesheet loads after the established design stylesheet', () => {
  const baseIndex = indexHtml.indexOf('css/style.css?v=294');
...
test('offline cache includes the responsive stylesheet', () => {
  ...
  assert.match(serviceWorker, /'\.\/js\/task-planning\.js'/);
  assert.match(serviceWorker, /'\.\/js\/data-compare\.js'/);
...
test('narrow Atlas and Knowledge controls keep their labels readable', () => {
  ...
  assert.match(indexHtml, /css\/style\.css\?v=294/);
