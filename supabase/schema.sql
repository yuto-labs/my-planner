...
--
-- このファイルは新しいSupabase環境を作るための「DB完成図」です。
-- create tableが保存場所、enable row level securityとcreate policyが
-- ユーザーごとのアクセス制限、create functionが複数テーブルを安全に扱う処理です。
-- 既存の本番環境を更新するときは、原則としてmigrations/の未適用SQLを使います。
-- ================================================================

-- ================================================================
-- BASE TABLES（新規環境で必要な基本テーブル）
-- ================================================================
-- ================================================================
...
create table if not exists ai_usage_events (
  ...
  user_id     uuid        not null references auth.users(id) on delete cascade,
  action_type text        not null default 'ai_request',
  cost        integer     not null default 1 check (cost > 0 and cost <= 20),
...
drop policy if exists "ai_usage_events: own read only" on ai_usage_events;
create policy "ai_usage_events: own read only" on ai_usage_events
  for select using (user_id = auth.uid());

drop function if exists claim_ai_usage(integer, text, integer, integer, integer);
